...
describe('OverlayTrigger', function() {
  ...
  describe('core functionality', () => {
    beforeEach(() => {
      const tooltip = (
        <Tooltip className="tooltip-text" content="Hello World" id="1"/>
      );
    ...
    it('pins the overlay if it is outside the window', function() {
      ...
      expect('.launcher').toHaveClass('tooltip-left');
      subject::setProps({pin: true});
      expect('.launcher').toHaveClass('tooltip-right');
    });
    ...
    describe('positioning', () => {
      ...
      it('works for top', () => {
        ...
        expect('.launcher').toHaveClass('tooltip-top');
      });
      it('works for bottom', () => {
        subject::setProps({placement: 'bottom'});
        expect('.launcher').toHaveClass('tooltip-bottom');
      });
      it('works for left', () => {
        subject::setProps({placement: 'left'});
        expect('.launcher').toHaveClass('tooltip-left');
  ...
  describe('when no id is provided', function() {
    beforeEach(function() {
      let tooltip = (
        <Tooltip content="Hello World3"/>
      );
      let tooltip2 = (
        <Tooltip content="Hello World4"/>
      );
    ...
    it('generates an id', function() {
      expect($('.tooltip-container')[0].id).not.toEqual('');
      expect($('.tooltip-container')[1].id).not.toEqual('');
    });

    it('id is unique', function() {
      expect($('.tooltip-container')[0].id).not.toEqual($('.tooltip-container')[1].id);
    });
  });
  ...
  describe('when an id is provided', function() {
    beforeEach(function() {
      const tooltip = (
        <Tooltip id="toiletpaper" content="Hello World2"/>
      );
    ...
    it('uses provided id', function() {
      expect('.tooltip-container').toHaveAttr('id', 'toiletpaper');
    });
