...
describe('FormUnit', () => {
  ...
  describe('inline', () => {
    ...
    it('renders the field and label on a grid next to each other', () => {
      expect('.form-unit .grid:eq(0) > .col:eq(0)'.text()).toBe('Instance Name');
      expect('.form-unit .grid:eq(0) > .col:eq(1)'.text()).toBe('hello');
    });
    ...
    it('renders the help row in a grid', () => {
      expect('.form-unit .grid:eq(1) > .col:eq(0).help-row'.text()).toBe('my-help-text');
    });
  });
  ...
  describe('postLabel', () => {
    ...
    describe('when the postLabel is a function', () => {
      ...
      beforeEach(() => {
        postLabel = jest.fn().and.returnValue(<span className="returned">returned</span>);
        setValues = jest.fn();
        state = {key: 'value'};
