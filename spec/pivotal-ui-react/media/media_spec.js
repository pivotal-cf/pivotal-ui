...
describe('Media', () => {
  ...
  describe('attributes', () => {
    ...
    it('creates a media component', () => {
      ...
      expect(subject.find(media.querySelector('.media-body')).text()).toContain('fop');
    });
  ...
  describe('when media is left-aligned', () => {
    ...
    it('adds the appropriate class based on media spacing', () => {
      ...
      expect(subject.find(result.find('.media').querySelector('.media-left')).hasClass('prs')).toBeTruthy();

      result = renderComponent({className: 'media-class', image, placement: 'left', mediaSpacing: 'medium'});
      expect(subject.find(result.find('.media').querySelector('.media-left')).hasClass('prm')).toBeTruthy();

      result = renderComponent({className: 'media-class', image, placement: 'left', mediaSpacing: 'large'});
      expect(subject.find(result.find('.media').querySelector('.media-left')).hasClass('prl')).toBeTruthy();

      result = renderComponent({className: 'media-class', image, placement: 'left', mediaSpacing: 'xlarge'});
      expect(subject.find(result.find('.media').querySelector('.media-left')).hasClass('prxl')).toBeTruthy();
    });
  });
  ...
  describe('when media is right-aligned', () => {
    ...
    it('adds the appropriate class based on media spacing', () => {
      ...
      expect(subject.find(result.find('.media').querySelector('.media-right')).hasClass('pls')).toBeTruthy();

      result = renderComponent({image, placement: 'right', mediaSpacing: 'medium'});
      expect(subject.find(result.find('.media').querySelector('.media-right')).hasClass('plm')).toBeTruthy();

      result = renderComponent({image, placement: 'right', mediaSpacing: 'large'});
      expect(subject.find(result.find('.media').querySelector('.media-right')).hasClass('pll')).toBeTruthy();

      result = renderComponent({image, placement: 'right', mediaSpacing: 'xlarge'});
      expect(subject.find(result.find('.media').querySelector('.media-right')).hasClass('plxl')).toBeTruthy();
    });
  });
  ...
  it('confers the appropriate classes to both children based on vAlign', () => {
    ['middle', 'bottom', 'top'].forEach(vAlign => {
      ...
      expect(subject.find(media.querySelector('.media-body')).hasClass(`media-${vAlign}`)).toBeTruthy();
      expect(subject.find(media.querySelector('.media-left')).hasClass(`media-${vAlign}`)).toBeTruthy();
    });
  });
  ...
  describe('when custom attributes are set on media', () => {
    it('passes them through', () => {
      ...
      expect(subject.find(media.querySelector('.media-body')).hasClass('inner-test-class')).toBeTruthy();
    });
  });
});
...
describe('Flag', () => {
  ...
  it('renders a Media component with vAlign set to middle, regardless of the passed-in vAlign value', () => {
    ...
    expect(subject.find(media.querySelector('.media-body')).hasClass('media-middle')).toBeTruthy();
    expect(subject.find(media.querySelector('.media-left')).hasClass('media-middle')).toBeTruthy();
  });
});