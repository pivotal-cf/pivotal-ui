...
describe('Modal', () => {
  ...
  it('renders a close button', () => {
    ...
    expect(subject.find(Icon).props()).toEqual({src: 'close', size: 'inherit', style: {}, verticalAlign: 'middle'});
  });
  ...
  describe('when given a title', () => {
    ...
    it('renders the title in a heading tag', () => {
      ...
      expect(
        subject.find('.pui-dialog.pui-modal-dialog .pui-modal-header h3.pui-modal-title').hasClass('em-high')
      ).toBeTruthy();
    });

    it('sets the aria-labelledby attribute on the dialog to be the ID of the title', () => {
      const titleId = subject.find('.pui-dialog.pui-modal-dialog .pui-modal-header h3.pui-modal-title').attr('id');
