...
describe('Flyout', () => {
  ...
  describe('header', () => {
    ...
    it('renders the specified icon', () => {
      expect(subject.find('.pui-flyout-header.grid > .col.col-fixed .pui-btn.pui-btn-default-flat.pui-btn-icon .icon.icon-middle .icon-chevron_left').text()).toBe('');
    });
    ...
    it('renders a header', () => {
      expect('.pui-flyout-header .col:eq(1)'.text()).toBe('some-flyout-header');
    });
    ...
    describe('when an icon src is provided', () => {
      ...
      it('renders that icon instead of the close icon', () => {
        expect(subject.find(`.pui-flyout-header.grid > .col.col-fixed .pui-btn.pui-btn-default-flat.pui-btn-icon .icon.icon-middle .icon-${iconSrc}`).text()).toBe('');
      });
    });
  });
});