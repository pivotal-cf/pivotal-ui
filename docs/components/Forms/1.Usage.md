...
Remember to provide an initial value. Also, when providing an `onChange` callback, stop the propagation of the browser event and use `setValues` to update the state.
...
<Form {...{
  ...
  fields: {
    url: {
      ...
      children: ({state: {current: {url: {host, path}}}, setValues}) => (
        <Grid>
          <FlexCol fixed>https://</FlexCol>
          <FlexCol>
            <Input value={host} onChange={e => {
              e.stopPropagation();
              setValues({url: {host: e.target.value, path}});
            }}/>
          </FlexCol>
          ...
          <FlexCol>
            <Input value={path} onChange={e => {
              e.stopPropagation();
              setValues({url: {host, path: e.target.value}});
            }}/>
          </FlexCol>
        </Grid>
      )
